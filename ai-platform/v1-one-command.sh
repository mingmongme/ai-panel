#!/usr/bin/env bash
# AI for You v1 — One-command installer
# Runs on the VPS directly. Downloads app from GitHub.
set -eu
...
INSTALLER_URL="https://raw.githubusercontent.com/mingmongme/ai-panel/main/ai-platform/install-v1.sh"
...
if ! command -v caddy &>/dev/null; then
  ...
  apt-get update -y && apt-get install -y debian-keyring debian-archive-keyring apt-transport-https curl 2>/dev/null || true
  curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/gpg.key' | gpg --dearmor -o /usr/share/keyrings/caddy-stable-archive-keyring.gpg 2>/dev/null || true
  curl -1sLf 'https://dl.cloudsmith.io/public/caddy/stable/debian.deb.txt' | tee /etc/apt/sources.list.d/caddy-stable.list >/dev/null
  apt-get update -y && apt-get install -y caddy 2>/dev/null || true
  ok "Caddy installed"
else
  ok "Caddy already installed"
fi

# Step 5: Download app from GitHub
status "Downloading app from GitHub..."
mkdir -p "$DEPLOY_DIR"
cd "$DEPLOY_DIR"
curl -fsSL "$TARBALL_URL" -o app.tar.gz || die "Download failed. Check GitHub access."
...
if [ -n "$EMAIL" ]; then
  ...
  certbot certonly --standalone --non-interactive --agree-tos --email "$EMAIL" -d "$DOMAIN" 2>/dev/null || warn "Certbot failed, trying caddy..."
